...
abstract class SServiceBase implements SMonitorBase {
    ...
    public function init()
    {
        ...
        $this->sthriftCli = new SThriftCli($this->host, $this->port, $this->persist, $this->genPath, $this->classNs, $this->rBufferSize, $this->wBufferSize);
    ...
    protected function call($itf, $func, array $params)
    {
        if ($this->isStarted)
        {
            $md5Val = md5($itf);
            ...
            {
                $this->itf[$md5Val] = new $itf($this->sthriftCli->getProtocol());
            }
            $startTime = microtime(true);
            try {
                $result = call_user_func_array(array($this->itf[$md5Val], $func), $params);
                $endTime = microtime(true);
                $this->onRemoteCallSuccess($this->serverName, $this->host, $this->port, $itf, $func, $params, isset($result->code) ? $result->code: 1, $endTime - $startTime);
                return $result;
            } catch (\Exception $ex) {
                $endTime = microtime(true);
                $this->onRemoteCallFailed($this->serverName, $this->host, $this->port, $itf, $func, $params, $ex->getMessage(), $endTime - $startTime);
                if ($this->downgradeCallEnable)
                {
                    $this->onLocalCall($this->serverName, $itf, $func, $params);
                    return $this->callDowngrade($itf, $func, $params);
                }
                else
                {
    ...
    protected abstract function callDowngrade($itf, $func, array $params);
    ...
    private function flushLog()
    {
        ...
        $logPath = $this->baseLogPath . DIRECTORY_SEPARATOR . 'thriftCall' . DIRECTORY_SEPARATOR . $this->classNs;
