...
abstract class TProtocol {
  ...
  public static function skipBinary($itrans, $type) {
    ...
    /**
     *
     * clean up this datapack when even is necessary
     *
     * by sergey <tsj@anzogame.com>
     *
     * @return int
    ...
    public function readClean()
    {
        ...
        $ftype = 0;
        $fid = 0;
        $xfer += $this->readStructBegin($fname);
        while (true)
        {
            $xfer += $this->readFieldBegin($fname, $ftype, $fid);
            if ($ftype == TType::STOP) {
                break;
            }
            switch ($fid)
            {
                default:
                    $xfer += $this->skip($ftype);
                    break;
            }
            $xfer += $this->readFieldEnd();
        }
        $xfer += $this->readStructEnd();
        return $xfer;
