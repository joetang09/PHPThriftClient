...
class TSocket extends TTransport {
  ...
  public function open() {
    ...
    if ($this->handle_ === FALSE) {
      $this->handle_=$this->reconnect();
      if($this->handle_ === FALSE)
      {
          $error = 'TSocket: Could not connect with retry to '.$this->host_.':'.$this->port_.' ('.$errstr.' ['.$errno.'])';
          if ($this->debug_) {
  ...
  public function close() {
     ...
     * Closes the socket.
     */
    public function closeForce() {

        $yii = '\Yii';
        $isInYii = class_exists($yii);
        if($isInYii && isset(\Yii::app()->params->thrift_lib_output) && \Yii::app()->params->thrift_lib_output){
            file_put_contents(\Yii::app()->params->thrift_lib_output, getmypid() . ' - close force ' . $this->handle_ . "\n", FILE_APPEND);
        }
...
//                $this->sendTimeoutSec_ + ($this->sendTimeoutUsec_ / 1000000));
            if ($this->handle_ === FALSE) {
                $error = 'TSocket: Could not reconnect to '.$this->host_.':'.$this->port_.' ('.$errstr.' ['.$errno.'])';
                if ($this->debug_) {
                    call_user_func($this->debugHandler_, $error);
                }
                throw new TException($error);
   ...
   */
  public function write($buf,$reconnect=true) {
    $rawBuf = $buf;
    $null = null;
    ...
    while (TStringFuncFactory::create()->strlen($buf) > 0) {
      ...
      if ($writable > 0) {
        ...
        $buf = TStringFuncFactory::create()->substr($buf, $written);
          ...
          if($reconnect==true) {
              $this->reconnect();
              return $this->write($rawBuf, false);
          }
          else {
              $this->closeForce();
